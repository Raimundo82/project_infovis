...
filterColumns(tracks, ['track_id', 'artist_id'])

tracks = tracks.merge(artists, left_on="artist_id", right_on="artist_id")

filterColumns(tracks, ['track_id', 'artist_id', 'genres'])

tracks=pd.DataFrame(tracks.assign(genres=tracks.genres.str.split(",")).explode('genres'))
tracks.tail()

filterColumns(tracks, ['track_id', 'genres'])
...
genres = tracks.genres.unique()
length = len(genres)

for genre in genres:

    genre_tracks=tracks[tracks['genres'] == genre]
    track_streams = genre_tracks.merge(streams, left_on="track_id", right_on="track_id")
    arr = []
    arr.append(genre)
    derived_measures(track_streams, arr)
    ...
    for year in range(2017,2021):
        ...
        genre_by_year = track_streams[track_streams['year'] == year]
        derived_measures(genre_by_year, arr)

        for month in range(1,13):
            if counter == 0:
                add_columns(columns, year=year, month=month)

            genre_by_month = genre_by_year[genre_by_year['month'] == month]

            derived_measures(genre_by_month, arr)
    final.append(arr)
    counter += 1
...
genre_derived_measures = pd.DataFrame(final, columns=columns)
genre_derived_measures.sort_values('all_chart_points', ascending=False, inplace=True)

genre_derived_measures.to_csv("project_datasets/global/genre_derived_measures.csv",index=False,header=True)
